...
public class RegularKit {
	...
	public static boolean isCode(String value){
		String check = "^[A-Z]{1,6}-[\\w-:\\*]+$";
		return match(check, Pattern.CASE_INSENSITIVE, value);
	}
	
	public static boolean isCode(String value, int min, int max){
		String check = "^[A-Z]{1,6}-[\\w-:\\*]{" + min + ","+ max + "}$";
		return match(check, Pattern.CASE_INSENSITIVE, value);
	}
	/**
	 * 判断是否是生日
	...
	public static void main(String[] args) {
		...
		String[] strs = { "http://www.13.com", "https://www.asdf.com.cn/asdf",
				...
				"ftp://192.168.0.2:8080", "http://ww.ss.com/////",
				"http://ww.gg.cn/" };
		for (int i = 0; i < strs.length; i++) {
			System.out
					.printf("%s --> %s%n", strs[i], RegularKit.isUrl(strs[i]));
		}
		System.out.println("--------------------------------");
		...
		for (int i = 0; i < strs.length; i++) {
			System.out.printf("%s --> %s%n", strs[i], pattern.matcher(strs[i])
					.find());
		}
		...
		System.out.println("UUID:"
				+ RegularKit.isUUID("ee706ee9-1658-43d0-bc1f-2db23cab4ab2"));
		
		System.out.println("En-Num:"
				+ RegularKit.isCode("PAAAAA-BASE:MOD-CC:M-UU:*"));
	}
}
