...
//	String m_strUploadReqPath = "/v2/index.php";
 ...
 //	int m_isTrans = 1;// 是否转码  0 ：不转    1：转
	private static int m_isTrans;
	int m_isScreenShot = 0;
//	int m_isWaterMark = 0;//是否加水印 0 ：不加    1：加
	private static int m_isWaterMark;
	private static int m_iClassId;
	int m_iExpireTime = 0;
	long m_qwFileSize = 0;
	...
	public VodCall(int isTranscode,int isWatermark,int m_iClassId) {
		VodCall.m_isTrans=isTranscode;
		VodCall.m_isWaterMark=isWatermark;
		VodCall.m_iClassId=m_iClassId;
	}
...
//			System.out.println(strMsg);
    ...
    public int Init(String secId, String secKey, int iUsage, int threadNum) {
		m_strSecId = secId;
		m_strSecKey = secKey;
		...
		} else if (iUsage == USAGE_UPLOAD || iUsage == USAGE_UGC_UPLOAD) {
	...
	@SuppressWarnings("static-access")
    public JSONObject CallRestApi(TreeMap<String, Object> mapVals) {
	...
	public int SetFileInfo(String strFilePath, String strFileName, String strFileType, int classId) {
    ...
    String GetReqSign(TreeMap<String, Object> mapVals) {
		...
		try {
			...
			if (m_iUsage == USAGE_UPLOAD) {
                contextStr += "POST";
            } else if (m_iUsage == USAGE_VOD_REST_API_CALL) {
                contextStr += "GET";
            }
...
//			getSign(s, m_strSecKey);
    ...
    String GetUgcExSign() {
		...
		contextStr += "&uid=" + "1";
		contextStr += "&tc=" + m_isTrans;
		contextStr += "&ss=" + this.m_isScreenShot;
		contextStr += "&wm=" + m_isWaterMark;
		
		if (m_iClassId != 0)
		{
			contextStr += "&cid=" + m_iClassId;
		}
...
//			int retCode = jsonobj.getInt("code");
			int retCode = jsonobj.getIntValue("code");
			if (retCode == 0) {
				partInfo.m_isSend = 1;
...
//				int canRetry = jsonobj.getInt("canRetry");
				int canRetry = jsonobj.getIntValue("canRetry");
...
//			int retCode = jsonobj.getInt("code");
			...
			if (retCode == 2) {
				...
				GeneratePartInfo();
				for (int i = 0; i < existPart.size(); i++) {
