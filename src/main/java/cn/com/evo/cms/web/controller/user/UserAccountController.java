...
public class UserAccountController extends BaseController {
	private static final String FORM_PAGE = "cms/user/userAccount/form";
	...
	protected UserAccountService getService() {
	...
	public DataResult list(HttpServletRequest request, Pager webPage) {
		...
		try {
			Specification<UserAccount> specification = DynamicSpecifications.bySearchFilter(request, UserAccount.class,null);
			List<UserAccount> entities = getService().findByCondition(specification, webPage);
			List<UserAccountVo> lstVo = Lists.newArrayList();
			for (UserAccount entity : entities) {
				UserAccountVo vo = mapper.map(entity, UserAccountVo.class);
				lstVo.add(vo);
			}
	...
	public ModelAndView add(HttpServletRequest request,@PathVariable("userId") String userId) {
	...
	@ResponseBody
	public MsgResult store(UserAccount entity) {
	...
	public ModelAndView edit(@PathVariable("id") String id) {
		ModelAndView mav = new ModelAndView(FORM_PAGE);
		UserAccount entity = getService().findById(id);
		mav.addObject("entity", entity);
		return mav;
	}
	...
	@ResponseBody
	public MsgResult modify(UserAccount entity) {
		MsgResult msgRet = new MsgResult();
		try {
