...
<div class="clearfix">
    <div class="grid3">
        <span class="grey">
            <i class="ace-icon fa fa-rmb fa-2x red"></i>
            &nbsp; 昨日订购金额
        </span>
        <h4 class="bigger pull-right" id="dayMoney">0</h4>
    </div>
    <div class="grid3">
        <span class="grey">
            <i class="ace-icon fa fa-male fa-2x purple"></i>
            &nbsp; 昨日订购用户数
        </span>
        <h4 class="bigger pull-right" id="dayUser">0</h4>
    </div>
    <div class="grid3">
        <span class="grey">
            <i class="ace-icon fa fa-rmb fa-2x red"></i>
        ...
        <h4 class="bigger pull-right" id="dayMaxMoney">0</h4>
    </div>
</div>
<div class="clearfix">
    <div class="grid3">
        ...
        <h4 class="bigger pull-right" id="allMoney">0</h4>
    </div>
    <div class="grid3">
        <span class="grey">
            <i class="ace-icon fa fa-male fa-2x purple"></i>
        ...
        <h4 class="bigger pull-right" id="allUser">0</h4>
    </div>
    <div class="grid3">
        <span class="grey">
            <i class="ace-icon fa fa-rmb fa-2x red"></i>
        ...
        <h4 class="bigger pull-right" id="allMaxMoney">0</h4>
    </div>
</div>
<div id="chartBox" style="margin:30px">
...
<script type="text/javascript">
    ...
    //获取统计数据
    // var Data = [[99, "#2dc6c8", "亲多多包月38元"], [58, "#b6a2dd", "亲多多包季128元"], [18, "#5ab1ee", "亲多多包年299元"], [6, "#d7797f", "亲多多包年499元"]];
    // goChart(Data);
    var ajaxListUrl = contextPath + '/home/all';
    $.post(ajaxListUrl, function (data) {
        if (data.status == 'OK') {
            //获取数据
            var rows = data.rows;
            var allMoney = rows.money;
            var allUser = rows.user;
            var allMaxMoney = rows.maxMoney;
            //页面赋值
            $('#allMoney').html(allMoney);
            $('#allUser').html(allUser);
            $('#allMaxMoney').html(allMaxMoney);

            //获取数据
            var dayUser = rows.dayUser;
            var dayMoney = rows.dayMoney;
            var dayMaxMoney = rows.dayMaxMoney;
            //页面赋值
            $('#dayUser').html(dayUser);
            $('#dayMoney').html(dayMoney);
            $('#dayMaxMoney').html(dayMaxMoney);

            var Data = rows.product;
            //饼状图 数据渲染
            goChart(Data);
        } else if (data.status == 'ERROR') {
            CmMsg.error(data.message, -1);
        } else {
            CmMsg.error("数据获取失败", -1);
        }
    });
