...
@Controller
@RequestMapping("/api/contentPictureMove")
public class ContentPictureMove extends BaseController {
	/**
	 * 连接
	 */
	...
	private void start() {
		...
		try {
			String sql = "select contentId,pictureId from cms_content_picture " + "ORDER BY createDate desc LIMIT "
					+ count + ",100";
	...
	private int getTotal() {
		int total = 0;
		try {
			String sql = "select count(*) from cms_content_picture";
